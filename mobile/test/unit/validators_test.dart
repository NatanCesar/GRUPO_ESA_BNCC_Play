...
void main() {
  group('Validators.email', () {
    ...
    test('recusa formato invalido', () {
      expect(Validators.email('maria'), 'E-mail inválido');
      expect(Validators.email('maria@'), 'E-mail inválido');
      expect(Validators.email('maria@escola'), 'E-mail inválido');
      expect(Validators.email('maria escola@x.com'), 'E-mail inválido');
    });
  ...
  group('Validators.usuario', () {
    ...
    test('cobra o preenchimento', () {
      expect(Validators.usuario(''), 'Informe seu nome de usuário');
    });

    test('recusa fora do tamanho', () {
      expect(
        Validators.usuario('ab'),
        'O nome de usuário precisa de 3 a 30 caracteres',
      );
      expect(
        Validators.usuario('a' * 31),
        'O nome de usuário precisa de 3 a 30 caracteres',
      );
    });

    test('recusa caractere especial e espaco', () {
      const msg = 'Use apenas letras, números, ponto e sublinhado';
  ...
  group('Validators.escola e Validators.turma', () {
    ...
    test('recusam fora do tamanho', () {
      ...
      expect(
        Validators.turma('a' * 81),
        'A turma precisa de 2 a 80 caracteres',
      );
    });
  });
  ...
  group('Validators.enunciado (Ciclo 2)', () {
    ...
    test('recusa enunciado longo', () {
      expect(
        Validators.enunciado('a' * 501),
        'Enunciado muito longo (máx 500 caracteres)',
      );
    });
  });
  ...
  group('Validators.opcaoQuestao (Ciclo 2)', () {
    ...
    test('cobra o preenchimento', () {
      expect(Validators.opcaoQuestao(''), 'Informe a opção');
      expect(Validators.opcaoQuestao('   '), 'Informe a opção');
      expect(Validators.opcaoQuestao(null), 'Informe a opção');
    });

    test('recusa opcao longa demais', () {
      expect(
        Validators.opcaoQuestao('a' * 201),
        'Opção muito longa (máx 200 caracteres)',
      );
    });
  });
}
